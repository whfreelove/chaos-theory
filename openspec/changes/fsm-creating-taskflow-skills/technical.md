...
- **Responsibilities**:
  ...
  - When the author accepts a split recommendation, apply deterministic task splitting: replace the pre-split task with the post-split tasks in the dependency graph. All parents of the pre-split task become parents of all post-split tasks, and all children of the pre-split task become children of all post-split tasks (node expansion — no functional change to dependency graph topology). Newly split tasks receive sequential IDs (max existing ID + 1 per new task). Warn the author that inherited dependency relationships may be imprecise and should be reviewed after splitting. Re-validate the dependency graph after splitting (no cycles introduced, all references valid)
...
- **Responsibilities**:
  ...
  - Guide the author on file placement: `plugins/<plugin>/skills/<skill>/fsm.json` alongside the SKILL.md. Check that the target directory exists and create it if it does not — this makes the component self-sufficient regardless of whether CMP-skill-md has already created the directory. Collision detection (offering overwrite/rename/abort for existing files) is handled by CMP-skill-md when it runs
