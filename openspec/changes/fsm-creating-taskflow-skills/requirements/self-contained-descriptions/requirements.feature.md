...
- Then the skill accepts the description without modification
- And the skill does not prompt the author to add additional context
...
- When the skill evaluates the description for self-containment
- Then the skill flags the missing context and prompts the author to incorporate the missing constraints into the description
- And the skill explains that the description must stand alone after the original skill text is compacted away
...
- When the skill evaluates the description for self-containment
- Then the skill flags the external reference
- And the skill instructs the author to replace the reference with the actual content it points to
- And the skill explains that context compaction removes the original skill text
...
- Then the skill re-evaluates the modified description for self-containment using the same rules (goal, constraints, expected outcome present; no external references; no inter-task references)
- And the skill does not accept the modification until the revised description passes all self-containment checks
...
- Then the skill presents tasks one at a time in dependency order
...
- Then the skill flags the reference to the named task
- And the skill instructs the author to inline the referenced information directly into the description
...
- When the skill evaluates the description for inter-task references
- Then the skill flags the sequential reference
- And the skill instructs the author to state the required preconditions explicitly within the description
- And the skill explains that task execution order is managed by dependencies, not by description text
...
- When the skill evaluates the description for inter-task references
- Then the skill flags the implicit assumption about prior task output
- And the skill instructs the author to either state the precondition explicitly or remove the assumption
...
- And the skill explains that smaller descriptions are more resilient to context compaction
...
- When the skill evaluates the description for appropriate sizing
- Then the skill flags the description as too granular to justify a separate task
- And the skill recommends merging the action into a related task where it contributes to a meaningful outcome
...
- When the skill evaluates the description for appropriate sizing
- Then the skill accepts the description without sizing-related modification
...
- Then the skill presents "Validating dependencies" as the proposed activeForm
...
- Then the skill accepts the author-provided activeForm as-is
- And the skill does not validate the override for present-continuous form
