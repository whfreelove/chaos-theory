...
- **Category**: delegate
- **Decision**: Update CMP-fsm-json-finalize responsibilities to include directory existence check with creation fallback, making the component self-sufficient regardless of CMP-skill-md execution order.
- **Outcome**: CMP-fsm-json-finalize file placement responsibility now checks that the target directory exists and creates it if absent, removing the cross-task assumption that CMP-skill-md has already run. Collision detection responsibility remains with CMP-skill-md. [technical.md CMP-fsm-json-finalize Responsibilities: replaced 'directory creation and collision detection are handled by CMP-skill-md' with self-sufficient directory existence check and creation fallback] Propagated to tasks.yaml: Entry 8 clause (g) updated to include directory existence check and creation before guiding file placement.
...
- **Category**: check-in
- **Decision**: Add a workflow-validation:1.8 requirement scenario specifying that when the in-context artifact is lost, the skill reconstructs from the most recent phase output and presents the reconstructed artifact to the author for verification.
- **Outcome**: Added workflow-validation:1.8 scenario 'Artifact recovery after in-context artifact loss' with Given/When/Then steps covering progressive construction artifact loss, reconstruction from most recent complete phase output, and author verification of the reconstructed artifact. [requirements/workflow-validation/requirements.feature.md Rule 1: added @workflow-validation:1.8 scenario after 1.7.1] Propagated to infra.md: workflow-validation Rule 1 coverage row updated to include artifact recovery scenario (1.8) in 'What it covers' and 'Verification approach' columns. Propagated to tasks.yaml: behavioral-verification workflow-validation Rule 1 checklist updated to include artifact recovery item covering workflow-validation:1.8.
...
- **Category**: check-in
- **Decision**: Delete dependency-mapping:4.1.1 as superseded by existing cycle detection scenarios (3.4 and workflow-validation:2.2). The observable cycle detection behavior is fully covered; 4.1.1 only describes an internal property. The topological sort detail already lives in technical.md.
- **Outcome**: Removed scenario 4.1.1 (Dependency graph supports cycle detection via topological sort). Observable cycle detection behavior is covered by dependency-mapping:3.4 (during mapping) and workflow-validation:2.2 (final validation). The topological sort implementation detail remains in technical.md.
...
- **Category**: check-in
- **Decision**: Split the Then clause into separate Then/And clauses with the skill as normative subject, removing the author-obligation clause.
- **Outcome**: Removed 'And the author SHALL confirm completeness before the next phase begins' from workflow-validation:1.7. The remaining two clauses ('Then the skill SHALL present a phase-completion summary...' and 'And the skill SHALL not advance to the next phase until the author confirms the summary') correctly place all normative obligations on the skill.
...
- **Category**: check-in
- **Decision**: Add dependency-mapping:2.3 scenario under Rule 2 (parallel execution patterns) covering the all-independent boundary case where multiple tasks have no dependencies between any of them.
- **Outcome**: Added scenario 2.3 'All tasks in a multi-task workflow are fully independent' under Rule 2. Covers four tasks with no inter-task dependencies, empty blockedBy arrays, dependency summary with no blocking relationships, and author confirmation of the fully parallel workflow. Propagated to tasks.yaml: Entry 6 coverage ref updated from 2.1-2.2 to 2.1-2.3 to include the new all-independent boundary case scenario.
...
- **Category**: check-in
- **Decision**: Defer-release. The 15-20 tasks threshold is a soft guideline with an arbitrary number. Document in functional.md Known Risks that requirements-level coverage for the threshold warning behavior is deferred to future usage validation.
- **Outcome**: Added sentence to the large-workflow Known Risk bullet: 'Requirements-level coverage for the large-workflow warning behavior is deferred to future usage validation.' Combined with GAP-199 rewrite of the same bullet.
...
- **Category**: check-in
- **Decision**: Co-resolve with GAP-207. Add phase-completion summary verification to infra.md workflow-validation Rule 1 coverage with pass/fail criteria and boundary examples. Pass: summary lists all tasks updated in the phase, author confirms. Fail: summary omits updated tasks or author identifies incompleteness.
- **Outcome**: Added phase-completion summary gate coverage to workflow-validation Rule 1 row in the coverage table (both 'What it covers' and 'Verification approach' columns) and added a 'Phase-completion summary (workflow-validation:1.7)' verification example section with 5 boundary scenarios defining pass/fail criteria for summary content, completeness, and detail level. Propagated to tasks.yaml: behavioral-verification workflow-validation Rule 1 checklist updated to include phase-completion summary items covering workflow-validation:1.7 and 1.7.1.
...
- **Category**: delegate
- **Decision**: Add concrete pass/fail boundary examples for the lightweight quality check to infra.md dependency-mapping Rule 5 coverage. Co-resolve with GAP-208.
- **Outcome**: Updated dependency-mapping Rule 5 coverage row to explicitly reference the lightweight quality check and its pass/fail boundary criteria in both columns. Added a 'Lightweight quality check for added tasks (dependency-mapping:5.2)' verification example section with 8 concrete pass/fail boundary examples illustrating label presence, description specificity, and actionability criteria.
...
- **Category**: check-in
- **Decision**: Append instruction to tasks.yaml entry 8 description: "After renumbering, present the old-to-new ID mapping to the author so they are aware of the ID changes."
- **Outcome**: Added old-to-new ID mapping presentation instruction to entry 8's renumbering clause (b), after the tie-breaking rule and before clause (c). Consistent with CMP-fsm-json-finalize responsibilities in technical.md.
...
- **Category**: check-in
- **Decision**: Add explicit retrieval instruction to tasks.yaml entry 9: "For the name-consistency check, read the SKILL.md file from the target directory (plugins/<plugin>/skills/<skill>/SKILL.md) and compare the frontmatter name field against the metadata.fsm value."
- **Outcome**: Added on-disk retrieval instruction to entry 9's name-consistency check (d), specifying the file path pattern and comparison target. Consistent with CMP-final-validation responsibilities in technical.md.
...
- **Category**: delegate
- **Decision**: Document as a Design Decision in technical.md — the auto-normalize step produces consistent directory name, metadata.fsm, and SKILL.md name values from the same normalized input. A 3-way mismatch requires deliberate author override. The 2-field check catches the most likely error (manual metadata editing). 3-way check deferred as low-risk.
- **Outcome**: Added Y-statement decision to the Decisions section explaining why the 2-field name-consistency check (metadata.fsm vs SKILL.md frontmatter name) is sufficient and a 3-way check including the directory name is deferred.
...
- **Category**: check-in
- **Decision**: Implement deterministic task splitting during description writing. The intra-split-task dependency ordering is known (split makes no sense otherwise). All parents of the pre-split task become parents of all post-split tasks, and all children of the pre-split task become children of all post-split tasks. No functional change to dependency graph topology (node expansion only). Warn author to clean up dependency relationships later if the inherited relationships are imprecise.
- **Outcome**: Added deterministic task splitting responsibility to CMP-descriptions defining node expansion mechanics, ID assignment for new tasks, author warning about imprecise inherited relationships, and post-split re-validation. Added corresponding entry to the Validation Scope table. Added Y-statement decision documenting the approach and neglected alternatives. Propagated to functional.md: self-contained-descriptions capability updated to mention task splitting during description writing, with inherited dependency relationships and post-split re-validation. Propagated to tasks.yaml: Entry 7 clause (g) updated with full deterministic split mechanics (parent/child inheritance, sequential ID assignment, author warning, re-validation).
...
- **Category**: check-in
- **Decision**: Rewrite capability descriptions in functional.md using author-observable terms. Replace "node addition, dangling reference cleanup, rename propagation" with "automatic updates to the dependency graph." Replace "self-containment feedback" with "feedback on each description's completeness during writing."
- **Outcome**: Rewrote dependency-mapping capability to use 'automatic updates to the dependency graph' instead of internal mechanism names. Rewrote workflow-validation capability to use 'feedback on each description's completeness' instead of 'self-containment feedback.'
...
- **Category**: check-in
- **Decision**: Rewrite Out of Scope entries in functional.md using user-facing language. Replace internal mechanism names with author-recognizable descriptions. Remove "deferred to implementation judgment" framing.
- **Outcome**: Rewrote six Out of Scope entries: replaced 'Precision of internal terminology in capability descriptions' with 'Exact wording of how capabilities are described to skill authors'; replaced 'phase-gate architecture' and 'Progressive Construction Protocol' references with author-facing descriptions; removed all 'deferred to implementation judgment' framing; replaced 'implementor determines' with 'body structure is flexible.'
...
- **Category**: check-in
- **Decision**: Rewrite Known Risks in functional.md using author-recognizable terms. Replace "15-20 tasks" with "Large workflows"; replace "phase-gate architecture" with "validates at each step"; replace "runtime guard" and "skill discovery directory" with author-facing descriptions.
- **Outcome**: Rewrote two Known Risk bullets: removed '15-20 tasks' threshold number, replaced 'runtime active-work-protection guard' with 'active-work protection', replaced 'skill discovery directory' with 'skill folder', replaced 'Phase-gate architecture' with 'The skill validates at each step before writing files.'
...
- **Category**: check-in
- **Decision**: Deprecated — document length is not a meaningful quality concern. Content belongs in its current location.
- **Rationale**: Document length is not a meaningful quality concern for prose-based spec files. The Brainstorming Gap Taxonomy is correctly placed in functional.md as a framework for evaluating brainstorming outputs — it is author-facing content. Extended Out of Scope lists provide explicit scope boundaries that benefit both authors and implementors. No evidence that length negatively impacts the spec's usefulness or correctness.
...
- **Category**: check-in
- **Decision**: Clarify functional.md language — the existing CMP-normalize confirmation gate is the discussion mechanism. The author reviews the concatenated list and resolves conflicts during the confirmation step. Update functional.md to make this explicit rather than implying a separate discussion mechanism.
- **Outcome**: Replaced 'combining inputs through discussion when material spans multiple sources' with 'reviewing and resolving overlaps during the normalization confirmation step when material spans multiple sources' in the workflow-intake capability description.
...
- **Category**: check-in
- **Decision**: Rewrite the Rule 5 preamble to lead with the skill obligation: "The skill SHALL support step list modifications during the dependency mapping phase. The author MAY add, remove, or rename tasks."
- **Outcome**: Rewrote Rule 5 preamble. Now leads with 'The skill SHALL support step list modifications during the dependency mapping phase' followed by 'The author MAY add, remove, or rename tasks while encoding dependencies.' The skill's normative obligation is explicit rather than implied.
...
- **Category**: delegate
- **Decision**: Rewrite to outcome-based language: "After any step list modification, the dependency graph SHALL reflect the change and referential integrity SHALL be maintained." Follows GAP-16/GAP-47 precedent.
- **Outcome**: Replaced 'Each modification SHALL trigger a dependency graph update and re-validation to maintain referential integrity' with 'After any step list modification, the dependency graph SHALL reflect the change and referential integrity SHALL be maintained.' Removes implementation mechanism language (trigger) in favor of observable outcome.
...
- **Category**: check-in
- **Decision**: Rewrite the Then clause to observable language: "the skill reports the issue and requests correction, then re-validates before completing." Follows GAP-146 precedent.
- **Outcome**: Removed the internal-state Then step 'the skill does not finalize SKILL.md until self-validation passes' from skill-file-generation:2.1. The remaining steps already express the same behavior in observable terms: the skill reports the failure, the author corrects it, and the skill re-validates before completing.
...
- **Category**: check-in
- **Decision**: Existing coverage is sufficient — workflow-intake:1.4 already covers the dual-active case with "one or both" phrasing, and infra.md Scenarios A-C cover three dual-source patterns. Gap concern addressed by existing specification.
- **Rationale**: workflow-intake:1.4 already covers the dual-active case with "one or both" phrasing, and infra.md Scenarios A-C cover three dual-source patterns. The existing specification is sufficient; the gap concern is addressed without requiring new scenarios.
...
- **Category**: check-in
- **Decision**: Add explicit Out of Scope entry to functional.md for author-initiated mid-phase cancellation. Author cancellation is generic conversational behavior, not a skill-specific concern. The existing confirmation gates handle this naturally — the skill waits for the author's response.
- **Outcome**: Added new Out of Scope entry: 'Author-initiated cancellation during a workflow phase — the existing confirmation gates naturally handle this; the skill waits for the author's response at each step.'
...
- **Category**: check-in
- **Decision**: Co-resolve with GAP-191. Add phase-completion summary verification to infra.md with pass/fail criteria and boundary examples.
- **Outcome**: Co-resolved with GAP-191. The Phase-completion summary verification example section includes boundary examples defining what constitutes a passing summary (all updated tasks listed with what changed), a failing summary (omitted tasks or insufficient detail), and the confirmation gate working as intended when the author identifies skipped tasks.
...
- **Category**: delegate
- **Decision**: Co-resolve with GAP-192. Single infra.md addition covers both boundary definition and verification examples for the lightweight quality check.
- **Outcome**: Co-resolved with GAP-192. The Lightweight quality check verification example section includes 8 concrete examples with clear pass, fail, and borderline verdicts illustrating where the specificity and actionability boundaries lie — including empty label, empty description, label-restating description, generic non-actionable descriptions, and borderline minimal-but-passing cases.
...
- **Category**: check-in
- **Decision**: Add dependency-mapping:5.5 as a standalone failure scenario for the lightweight quality check: skill reports which criterion failed, prompts the author to correct the submission, and withholds the task from the graph until correction.
- **Outcome**: Added scenario 5.5 'Lightweight quality check fails for a newly added task' under Rule 5. Covers the failure path: skill reports which quality criterion failed, prompts the author to correct the submission, and withholds the task from the step list and dependency graph until the quality check passes. Propagated to technical.md: CMP-dependency-map Responsibilities (lightweight quality check bullet) — added explicit failure path sentence before the existing 'prevents clearly under-specified tasks' closing sentence. Propagated to tasks.yaml: Entry 6 coverage ref updated to add dependency-mapping:5.5.
...
- **Category**: delegate
- **Decision**: Add workflow-validation:1.7.1 failure sub-scenario for when the phase-completion summary reveals incomplete or skipped entries. The skill reports which entries are missing and blocks advancement.
- **Outcome**: Added workflow-validation:1.7.1 scenario 'Advancement blocked when phase-completion summary reveals incomplete entries' specifying that the skill reports which entries were not updated and blocks advancement until incomplete entries are addressed. Propagated to infra.md: workflow-validation Rule 1 coverage table updated to include advancement blocked when summary reveals incomplete entries (1.7.1) in 'What it covers' and 'Verification approach' columns.
...
- **Category**: check-in
- **Decision**: Defer-resolution — user feedback indicates tasks.yaml has too much implementation detail; adding more checklist items is counterproductive.
- **Outcome**: No artifact changes. Gap deferred to a future resolution round based on user feedback that tasks.yaml behavioral-verification checklists have too much implementation detail.
...
- **Category**: delegate
- **Decision**: Defer-release — consistent with user feedback that tasks.yaml has too much implementation detail.
- **Outcome**: No artifact changes. Gap deferred to a future release based on user feedback that tasks.yaml behavioral-verification checklists have too much implementation detail.
