...
In the context of circular dependency detection at runtime, facing implementation complexity for cycle detection in task graphs, we decided not to detect circular dependencies at runtime, to achieve implementation simplicity and behavioral consistency with Claude Code's task system (which also does not detect cycles), accepting that circular dependencies will silently create unresolvable blockedBy chains. This decision applies to runtime execution only — authoring-time tooling (e.g., the creating-taskflow-skills skill) may perform cycle detection during workflow construction. `[finite-skill-machine]`
